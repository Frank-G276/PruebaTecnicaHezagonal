...
public class CreateTransferTransactionService implements CreateTransferTransactionUseCase {
    ...
    public CreateTransferTransactionService(UpdateBalancesProductUseCase updateBalancesProductUseCase, TransactionRepository transactionRepository) {
        this.updateBalancesProductUseCase = updateBalancesProductUseCase;
        this.transactionRepository = transactionRepository;
    }

    @Override
    public TransferResultDto execute(CreateTransactionDto transactionDto) {
        Balance balanceOriginProduct = updateBalancesProductUseCase.execute(
                new UpdateBalanceDto(
                        TypeTransactionEnum.WITHDRAWAL,
                        new Amount(transactionDto.amount())),
                transactionDto.originIdProduct()
        );
        Balance balanceDestinationProduct = updateBalancesProductUseCase.execute(
                new UpdateBalanceDto(
                        TypeTransactionEnum.DEPOSIT,
                        new Amount(transactionDto.amount())
                ),
                transactionDto.destinationIdProduct()
        );

        Transaction destinationTransaction = new Transaction(
                new IdProduct(transactionDto.destinationIdProduct()),
                new IdProduct(null),
                new Amount(transactionDto.amount()),
                new Description(transactionDto.description()),
                new Balance(balanceDestinationProduct.getBalance()),
                TypeTransactionEnum.WITHDRAWAL
        );
        ...
        return new TransferResultDto(originTransaction, destinationTransaction);
    }
}
